...
# --- MODIFICATION RUNPOD ---
# Installation des dépendances nécessaires pour la détection GPU
...
# --- MODIFICATION RUNPOD ---
# Définition manuelle des dossiers cibles
OLLAMA_INSTALL_DIR="/workspace/ollama"
BINDIR="$OLLAMA_INSTALL_DIR/bin"
MODELS_DIR="$OLLAMA_INSTALL_DIR/models"

status "Creating custom directories..."
$SUDO mkdir -p "$BINDIR"
$SUDO mkdir -p "$MODELS_DIR"

# On s'assure que les variables d'environnement sont définies pour la session courante du script
...
# Création d'un lien symbolique global si possible (optionnel, pratique pour appeler 'ollama' sans le chemin complet)
...
install_success() {
    ...
    status "Install complete. Custom models directory: $MODELS_DIR"
    status "IMPORTANT: To run ollama with these models, use:"
    status "export OLLAMA_MODELS=$MODELS_DIR"
    status "$BINDIR/ollama serve"
}
...
configure_systemd() {
    ...
    # MODIFICATION: Ajout de Environment="OLLAMA_MODELS=..." dans le service systemd
...
# ... (Le reste du script pour l'installation manuelle CUDA reste inchangé car il est rarement utilisé sur RunPod qui a déjà les drivers)
# Je garde le reste du script intact pour la cohérence.
